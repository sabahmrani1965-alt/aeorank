...
export async function getEarningsSummary(admin, posterId) {
  ...
  const taskRows = (tasks || []).map((t) => ({ ...t, rate: rateForType(t.type), isBonus: false }));
  ...
  const totalEarned = rows.reduce((sum, r) => sum + r.rate, 0);
  ...
  const tasksWithPaidStatus = rows
    .map((r) => {
      ...
      return {
        ...r,
        subreddit: r.isBonus ? r.subreddit : displaySubreddit(r.subreddit),
        displayStatus: isPaid ? "paid" : "submitted",
      };
    })
