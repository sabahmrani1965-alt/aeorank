// Pure string-utility helpers only — safe to bundle for the browser
// (components/DashboardAnalyzeForm.js, a client component, imports
// urlToSlug from here). The actual network fetch (fetchSiteMeta) lives in
// lib/siteFetch.js, a server-only module using node:dns/node:net for SSRF
// protection, which can't be bundled client-side.

function pick(html, regex) {
  const m = html.match(regex);
  return m ? decodeEntities(m[1].trim()) : "";
}

function decodeEntities(s) {
  return s
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
    .replace(/&nbsp;/g, " ");
}

// Finds the first <link> tag whose rel attribute matches relPattern
// (case-insensitive, order-independent — real sites put rel/href in
// either order) and returns its href, or "" if none match. Matched as two
// separate passes rather than one combined regex, since attribute order
// in real HTML isn't reliable enough to assume rel always comes before
// href.
function pickLinkHref(html, relPattern) {
  const linkTags = html.match(/<link\b[^>]*>/gi) || [];
  for (const tag of linkTags) {
    if (!relPattern.test(tag)) continue;
    const hrefMatch = tag.match(/href=["']([^"']+)["']/i);
    if (hrefMatch) return decodeEntities(hrefMatch[1].trim());
  }
  return "";
}

export function parseMeta(html, baseUrl) {
  const title =
    pick(html, /<title[^>]*>([\s\S]*?)<\/title>/i) ||
    pick(html, /<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i);
  const description =
    pick(html, /<meta[^>]+name=["']description["'][^>]+content=["']([^"']+)["']/i) ||
    pick(html, /<meta[^>]+property=["']og:description["'][^>]+content=["']([^"']+)["']/i) ||
    pick(html, /<meta[^>]+name=["']twitter:description["'][^>]+content=["']([^"']+)["']/i);

  // Prefer an actual icon over og:image — og:image is usually a wide
  // social-share banner, not a square logo/avatar. apple-touch-icon is
  // typically the highest-resolution real icon a site ships.
  let logo =
    pickLinkHref(html, /rel=["']apple-touch-icon(?:\s+precomposed)?["']/i) ||
    pickLinkHref(html, /rel=["'](?:shortcut\s+)?icon["']/i) ||
    pick(html, /<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i);
  if (logo && baseUrl) {
    try {
      logo = new URL(logo, baseUrl).toString();
    } catch {
      logo = "";
    }
  } else if (!baseUrl) {
    logo = ""; // a relative path with nothing to resolve against is useless
  }

  return { title, description, logo, ok: true };
}

// Derive a URL-safe report slug from whatever a visitor typed in
// ("notion.so", "https://www.notion.so/product") — used to build the
// /report/[slug] path.
export function urlToSlug(input) {
  let url = String(input || "").trim();
  if (!url) return "";
  if (!/^https?:\/\//i.test(url)) url = "https://" + url;
  try {
    const u = new URL(url);
    const host = u.hostname.replace(/^www\./, "");
    const base = host.split(".")[0];
    return base.toLowerCase().replace(/[^a-z0-9]/g, "");
  } catch {
    return String(input || "").toLowerCase().replace(/[^a-z0-9]/g, "");
  }
}

// Convert a slug like "saasoffers" or "notionhq" into something nicer.
// Handles common compound patterns: "saasoffers" → "SaaSOffers", "notionhq" → "NotionHQ".
// Also detects acronym-style slugs (3+ chars, all consonants) → uppercase: "ffn" → "FFN".
export function prettyBrand(slug) {
  if (!slug) return "Your brand";
  const lower = slug.toLowerCase();

  // Pure acronym detection: short, all consonants (3-5 chars).
  if (lower.length >= 2 && lower.length <= 5) {
    const vowels = (lower.match(/[aeiouy]/g) || []).length;
    if (vowels === 0) return lower.toUpperCase(); // ffn → FFN
  }

  // Compound brands: split on known sub-words and re-cap.
  const KNOWN = ["saas", "club", "offers", "hub", "labs", "app", "ai", "kit", "stack", "pad", "deck", "base", "lab", "box", "hq"];
  let s = lower;
  for (const w of KNOWN) {
    s = s.replace(new RegExp(`(?!^)${w}`, "g"), (mm) => "_" + mm);
  }
  return s
    .split("_")
    .filter(Boolean)
    .map((part) => {
      if (part.toLowerCase() === "saas") return "SaaS";
      if (part.toLowerCase() === "ai") return "AI";
      if (part.toLowerCase() === "hq") return "HQ";
      return part.charAt(0).toUpperCase() + part.slice(1);
    })
    .join("");
}

// When the page title is available, try to pull a clean brand name from it.
// Many sites use formats like:
//   "Fédération Française de Natation | FFN"
//   "Notion – The AI workspace that works for you"
//   "Shopify - Start, run, and grow your business"
//   "GitHub: Let's build from here"
// We split on common separators and prefer the shortest meaningful chunk.
export function extractBrandFromTitle(title) {
  if (!title) return null;
  const cleaned = title.trim();
  // Split on common title separators: " | ", " - ", " – ", " — ", " · ", ": "
  // (colon doesn't require leading space — handles "SaaSOffers: tagline")
  const parts = cleaned
    .split(/\s*[|\-–—·]\s+|:\s+/)
    .map((p) => p.trim())
    .map((p) => p.replace(/\.(fr|com|org|net|io|co|de|es|it|uk)$/i, "")) // strip trailing TLDs
    .filter(Boolean);
  if (parts.length === 0) return null;
  // Prefer the shortest meaningful segment (usually the brand)
  const sorted = parts.slice().sort((a, b) => a.length - b.length);
  const shortest = sorted[0];
  if (shortest.length >= 2 && shortest.length <= 32) return shortest;
  // Fall back to first 5 words
  return parts[0].split(/\s+/).slice(0, 5).join(" ");
}
