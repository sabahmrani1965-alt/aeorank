...
export const CREDIT_COSTS = {
  ...
  generate_reply: 5,
  generate_post: 10,
  // Upvotes are priced as a bundle, not a flat per-unit rate — see
  // UPVOTES_PER_CREDIT/upvoteCreditCost below. This value is credits per
  // bundle (currently 1), not credits per upvote.
  generate_upvote: 1,
  rewrite_draft: 1,
...
// 10 upvotes per 1 credit. Rounded up so an order that isn't an exact
// multiple of the bundle size still costs a whole credit for the partial
// bundle, rather than under-charging.
export const UPVOTES_PER_CREDIT = 10;
export function upvoteCreditCost(qty) {
  return Math.ceil(qty / UPVOTES_PER_CREDIT) * CREDIT_COSTS.generate_upvote;
}
