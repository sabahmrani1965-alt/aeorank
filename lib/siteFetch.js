...
export async function fetchSiteMeta(url) {
  ...
  try {
    ...
    // currentUrl (not the original `url`) — the final hop after any
    // redirects, so a relative icon href in the HTML resolves against
    // where the page actually lives, not where the visitor started.
    return parseMeta(html, currentUrl);
  } catch (e) {
    return { title: "", description: "", logo: "", ok: false, error: String(e.message || e) };
  } finally {
    clearTimeout(timer);
  }
}
