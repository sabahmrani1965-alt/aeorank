...
export async function analyzeBrandVisibility(brand, categoryQuery) {
  ...
  // Cloro-backed engines answer only the FIRST (strongest "name specific
  // products") query, not all three: Cloro calls are 10-45s each and its
  // plan tier caps concurrency, so engines × queries through it would
  // blow both the page's time budget and the rate limit. One real answer
  // per engine still gives the report all four engine names honestly.
  if (isCloroConfigured()) {
    const q = queries[0];
    if (!process.env.GEMINI_API_KEY) {
