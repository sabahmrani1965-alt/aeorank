...
//
// Shared by the direct admin-invite route (referredBy/redditUsername/
// redditCheckStatus: null) and the apply-poster approval route
// (referredBy: the referring poster's id; redditUsername/
// redditCheckStatus: from the application's verified Reddit account).
export async function createOrPromotePoster({
  email,
  referredBy = null,
  redditUsername = null,
  redditCheckStatus = null,
  origin,
}) {
  ...
  const { error: roleError } = await admin
    ...
    .update({
      ...
      ...(redditUsername ? { reddit_username: redditUsername } : {}),
      ...(redditCheckStatus ? { reddit_check_status: redditCheckStatus } : {}),
    })
    .eq("id", userId);
  if (roleError) {
  ...
  if (isNewAccount) {
    ...
    const html = `
      <div style="font-family:ui-sans-serif,system-ui,sans-serif;color:#0d0e1d;line-height:1.55">
        <h2 style="margin:0 0 12px">You've been added as a CrewQuest poster</h2>
        <p>You can now log in to pick tasks and start earning.</p>
        ...
        <p><a href="${esc(loginUrl)}" style="color:#ff4500">Log in →</a></p>
    ...
    `;
    const text =
      `You've been added as a CrewQuest poster.\n\n` +
      `Email: ${email}\nTemporary password: ${tempPassword}\n\n` +
      `Log in: ${loginUrl}\n\nYou can change this password any time via "Forgot password" on the login page.`;

    const result = isEmailConfigured()
      ? await sendEmail({ to: email, subject: "Your CrewQuest poster account", html, text })
